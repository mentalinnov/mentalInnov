...
async function handleInput({ flags }) {
    if (flags.pack) {
        const { root, filename } = await fs_1.packToFs({ input: flags.pack, output: flags.output });
        ...
        if (flags.unpack === '') {
            return fs_2.unpackStreamToFs({ input: process.stdin, roots, output: flags.output });
        }
        return fs_2.unpackToFs({ input: flags.unpack, roots, output: flags.output });
    }
    else if (flags.list) {
        return lib_js_1.listFilesInCar({ input: flags.list });
    }
    else if (flags.listRoots) {
        return lib_js_1.listRootsInCar({ input: flags.listRoots });
    }
    else if (flags.listCids) {
        return lib_js_1.listCidsInCar({ input: flags.listCids });
    }
    else if (!process.stdin.isTTY) {
        // maybe stream?
        // tslint:disable-next-line: no-console
        console.log('Reading .car from stdin');
        return fs_2.unpackStreamToFs({ input: process.stdin, output: flags.output });
    }
    else {
        cli.showHelp();
