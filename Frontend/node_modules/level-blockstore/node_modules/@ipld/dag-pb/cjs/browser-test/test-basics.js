...
describe('Basics', () => {
  ...
  it('prepare & encode a node with data', () => {
    ...
    const prepared = index.prepare({ Data: data });
  ...
  it('prepare & encode a node with links', () => {
    ...
    const prepared = index.prepare({ Links: links });
  ...
  it('ignore invalid properties when preparing', () => {
    const prepared = index.prepare({ foo: 'bar' });
    assert.deepEqual(prepared, { Links: [] });
    const result = index.encode(prepared);
    assert.strictEqual(result.length, 0);
  });
  ...
  it('prepare & create a node with bare string', () => {
    const data = 'some data';
    const prepared = index.prepare(data);
    assert.deepEqual(prepared, {
      Data: new TextEncoder().encode(data),
      Links: []
    });
  ...
  it('prepare & create a node with links (& sorting)', () => {
    ...
    const prepared = index.prepare(node);
  ...
  it('prepare & create a node with bytes only', () => {
    ...
    const reconstituted = index.decode(index.encode(index.prepare(node)));
  ...
  it('fail to prepare & create a node with other data types', () => {
    ...
    for (const invalid of invalids) {
      assert.throws(() => index.encode(index.prepare(invalid)), 'Invalid DAG-PB form');
    }
  });
  it('fail to prepare & create a link with other data types', () => {
    ...
    for (const invalid of invalids) {
      assert.throws(() => index.encode(index.prepare({ Links: [invalid] })), 'Invalid DAG-PB form');
    }
  });
  it('fail to create link with bad CID hash', () => {
    assert.throws(() => index.prepare({
      Links: [{
  ...
  it('fail to create without hash', () => {
    ...
    assert.throws(() => index.prepare(node), 'Invalid DAG-PB form');
  });
});
